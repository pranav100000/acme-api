 ...
 * Provides async CRUD operations for users and teams with simulated latency
 * (10ms delays) to mimic real database behavior. Data resets on server restart.
 */
...
// Deep-copy initial state so _reset() can restore the database for tests
...
const db = {
  /** Find a single user by their unique ID. Returns null if not found. */
  async findUser(id) {
    await new Promise(resolve => setTimeout(resolve, 10));
    return users.find(u => u.id === id) || null;
  },

  /** Look up a user by email address (used for login and duplicate checks). */
  ...
  /** Find a single team by ID. Returns null if not found. */
  async findTeam(id) {
    await new Promise(resolve => setTimeout(resolve, 10));
  ...
  /** Resolve a team's member IDs to full user objects. Returns null if team not found. */
  ...
  /** Create a new team with an empty members list. */
  ...
  /** Add a user to a team (no-op if already a member). Returns null if team or user not found. */
