...
describe('User Routes', () => {
  ...
  // BUG: These endpoints don't check if user exists before accessing properties,
  // causing a TypeError (500) instead of returning a proper 404 response.
  test('GET /api/users/:id crashes for non-existent user (known bug)', async () => {
    const res = await fetch(`${baseUrl}/api/users/999`);
    assert.strictEqual(res.status, 500);
  });

  test('GET /api/users/:id/profile crashes for non-existent user (known bug)', async () => {
